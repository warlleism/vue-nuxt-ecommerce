<template>
    <div 
    id="search"
    class="flex items-center justify-between h-10 overflow-hidden rounded-md border border-gray-300 ">
        ...
        <div v-if="searchResults.length > 0"
            ...
            <NuxtLink @click="searchResults = []" :to="`/products/${product.id}`" v-for="product in searchResults"
                :key="product.id" class="flex flex-row justify-between items-center gap-2 border-b border-[#ecedf0] pb-2">
                ...
                <p v-if="product.discount" class="bg-orange-500 text-white font-semibold rounded-md p-1">-{{ product.discount }}</p>
            </NuxtLink>
            <div class="flex items-center justify-start">
                <div class="text-orange-500 font-semibold">Ver todos...</div>
            </div>
        </div>
    </div>
</template>
